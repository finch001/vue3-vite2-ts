...
import { NSpace, NPopconfirm, NInput, NButton, NCheckbox } from 'naive-ui';
  ...
  setup(props) {
    ...
    const renderActions = (item: DataItem): VNodeChild => (
      <NSpace>
        {(item.editing && (
          <>
            <NButton onClick={() => finishEdit(item)} type="primary">
              保存
            </NButton>
          ...
          <>
            ...
            <NButton onClick={() => edit(item)} type="warning" ghost>
              编辑
            </NButton>
          </>
        )}
    ...
    let columns = [
      {
        ...
        render: (item: RowData) => {
          return (
            <NCheckbox
              v-model={[(item as any as DataItem).finish, 'checked']}
            ></NCheckbox>
          );
        }
      },
      ...
      },
      {
        ...
        render: (item: RowData): VNodeChild => {
          return item.editing ? (
            <NInput v-model={[item.title, 'value']} />
          ) : (
